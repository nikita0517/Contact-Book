...
class LoginWindow(Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.username_entry = Entry(login_frame, font = ('Arial', 15), width = 12)
        self.username_entry.grid(row = 0, column = 1, pady = 5)

        password_label = Label(login_frame, text = "Password:", style = 'Login.TLabel')
        password_label.grid(row = 1, column = 0)

        self.password_entry = Entry(login_frame, font = ('Arial', 15), width = 12, show = '*')
        self.password_entry.grid(row = 1, column = 1, pady = 5)

        s.configure('Login.TButton', font = ('Arial', 15))

        login_button = Button(login_frame, text = "Login", width = 12,
